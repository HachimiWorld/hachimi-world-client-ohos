...
class OhosPlatform : Platform {
    ...
    override val userAgent: String get() = "HachimiWorld-ohos/${BuildKonfig.VERSION_NAME} ($platformVersion)"
    ...
    override fun openUrl(url: String) {
        ...
        OhosUrlLauncher.send("openUrl", url)
    }
}

actual fun getPlatform(): Platform = OhosPlatform()

internal fun sendOhosPlatformAction(operation: String, value: String) = OhosUrlLauncher.send(operation, value)
...
private object OhosUrlLauncher {
    ...
    fun send(operation: String, value: String) {
        scope.launch {
            try {
                ...
                memScoped {
                    ...
                    try {
                        ...
                        check(napi_create_string_utf8(env, operation, operation.encodeToByteArray().size.convert(), arguments) == napi_ok)
                        check(napi_create_string_utf8(env, value, value.encodeToByteArray().size.convert(), arguments + 1) == napi_ok)
                        val status = napi_call_function(env, receiver.value, function.value, 2u, arguments, null)
